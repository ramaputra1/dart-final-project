...
class CommandRunner {
  // constructor accepts the optional callback
  ...
  ArgResults parse(List<String> input) {
    ...
    // it helps to throw an exception if the command is not recognized
    ...
    // exception if multiple commands are provided
    if (results.command != null &&
        input.isNotEmpty &&
    ...
    while (i < input.length) {
      if (input[i].startsWith('-')) {
        ...
        // exception if an option is not recognized for the given command
        ...
        if (option.type == OptionType.option) {
          // exception if an option requires an argument but none is given
          if (i + 1 >= input.length) {
            throw ArgumentException(
